...
class _EmptyPlanningProvider:
    def find_projects(self, _query: str | Iterable[str], **_options: Any) -> dict[str, str]:
        return {}
    ...
    def resolve_files(
        ...
        **options: Any,
        ...
        if options.get("latest") and not options.get("minecraft_version") and not options.get("loader"):
            return {}
        return {}
...
def _review_planning(build_dependency_plan=None) -> Any:
    ...
    return SimpleNamespace(
        provider=SimpleNamespace(find_projects=lambda _query, **_options: {}),
...
def _queue_feature(
    ...
    *,
    ...
    controller = SimpleNamespace(
        mod_session=session,
        online_browse_presenter=presenter,
        format_online_environment=lambda _mod: "未知",
    )
...
def test_on_online_browse_filters_changed_refreshes_hint_and_reloads(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(feature, "refresh_online_filter_hint", lambda: called.append("hint"))
    monkeypatch.setattr(feature, "refresh_online_results_summary", lambda: called.append("summary"))
    monkeypatch.setattr(
        feature,
        "_load_online_mods",
    ...
    feature.on_online_browse_filters_changed()
...
def test_copy_online_mod_info_handles_clipboard_failure(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    controller.format_online_environment = lambda _mod: "Fabric / server"
...
def test_refresh_local_list_keeps_full_description(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(projection, "capture_selected_mod_ids", _capture_selected_mod_ids_func)
    ...
    monkeypatch.setattr(tree_sync_module, "QSignalBlocker", lambda _tree: nullcontext())
...
def test_reveal_in_explorer_uses_windows_select_argument(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(ui_module.UIUtils, "_is_safe_windows_path_argument", lambda _path: True)
    monkeypatch.setattr(utils_module.SubprocessUtils, "run_checked", fake_run_checked)

    ui_module.UIUtils.reveal_in_explorer(Path("C:/servers/Alpha/mods/example.jar"))
...
def test_delete_local_mod_delegates_to_mod_manager_and_refreshes(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_ask_yes_no_cancel(_title, _message, **_kwargs) -> bool:
        return True

    monkeypatch.setattr(ui_module.UIUtils, "ask_yes_no_cancel", fake_ask_yes_no_cancel)
    monkeypatch.setattr(
        ui_module.UIUtils,
        "show_message",
...
def test_install_pending_online_install_queue_deduplicates_shared_dependencies(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    controller.refresh_online_queue = lambda: None
    ...
    class _ImmediateScope:
        def schedule(self, _delay_ms, callback, *args, **_kwargs):
            callback(*args)
    ...
    monkeypatch.setattr(ui_module.UIUtils, "ask_yes_no_cancel", _confirm_dialog)
    monkeypatch.setattr(ui_module.UIUtils, "show_message", _show_message)
...
def test_add_pending_online_install_blocks_client_only_mod(monkeypatch) -> None:
    ...
    controller.parent = SimpleNamespace()
    controller.update_status_safe = lambda _message: None
    monkeypatch.setattr(feature, "refresh_online_queue", lambda: None)
...
def test_add_pending_online_install_replaces_same_version_item(monkeypatch) -> None:
    ...
    controller.update_status_safe = lambda _message: None
    monkeypatch.setattr(feature, "refresh_online_queue", lambda: None)
    monkeypatch.setattr(ui_module.UIUtils, "show_message", lambda *_args, **_kwargs: None)
