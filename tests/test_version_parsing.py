...
def test_server_inspector_bounded_reader_rejects_oversized_directory_version_json(tmp_path) -> None:
    version_json = tmp_path / "version.json"
    version_json.write_bytes(b'{"id":"1.21.1"}' + b" " * ARCHIVE_METADATA_MAX_BYTES)

    assert read_json(version_json, max_bytes=ARCHIVE_METADATA_MAX_BYTES) is None


@pytest.mark.parametrize(
    ("input_text", "expected"),
    [
        ("minecraft-1.20.4-server.jar", "1.20.4"),
        ("mc_1.21.1", "1.21.1"),
        ("version 1.19.2", "1.19.2"),
        ("1.20.2-rc1", "1.20.2-rc1"),
        ("1.20-pre2", "1.20-pre2"),
        ("1.20-snapshot-123", "1.20-snapshot-123"),
        ("23w45a", "23w45a"),
        ("server-1.20.1.jar", "1.20.1"),
        ("no version here", None),
        ("", None),
    ],
)
def test_extract_minecraft_version_from_text(input_text: str, expected: str | None) -> None:
    assert extract_minecraft_version_from_text(input_text) == expected
