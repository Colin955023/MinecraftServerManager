...
def test_matches_known_sha256(tmp_path: Path) -> None:
    ...
    actual = compute_file_hash_sync(file_path, algorithm="sha256")
...
def test_matches_known_sha512(tmp_path: Path) -> None:
    ...
    actual = compute_file_hash_sync(file_path, algorithm="sha512")
...
def test_unsupported_algorithm_returns_empty_string(tmp_path: Path) -> None:
    ...
    assert compute_file_hash_sync(file_path, algorithm="not-a-real-algorithm") == ""
...
def test_missing_file_returns_empty_string(tmp_path: Path) -> None:
    ...
    assert compute_file_hash_sync(missing_path, algorithm="sha256") == ""
...
def test_chunked_reading_matches_single_read(tmp_path: Path) -> None:
    ...
    hash_small_chunks = compute_file_hash_sync(file_path, algorithm="sha256", chunk_size=17)
    hash_default_chunk = compute_file_hash_sync(file_path, algorithm="sha256")

    assert hash_small_chunks == hash_default_chunk == hashlib.sha256(content).hexdigest()
