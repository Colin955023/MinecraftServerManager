...
class _Combo:
    ...
    def addItem(self, _text: str) -> None:
        return None

    def setEnabled(self, enabled: bool) -> None:
        self.enabled = enabled

    def setCurrentText(self, _text: str) -> None:
        return None
...
def test_switching_from_vanilla_clears_stale_loader_version_before_background_load() -> None:
    frame = _make_frame("1.21.1", loader_type="Vanilla", mc_version="1.21.1")
    CreateServerFrame.update_server_config_ui(frame)
    assert frame.loader_version_var.get() == "1.21.1"

    frame.loader_type_var.set("Fabric")
    CreateServerFrame.update_server_config_ui(frame)

    assert frame.loader_version_var.get() == "正在載入 Fabric 版本..."
    assert frame.loader_version_combo.enabled is False
