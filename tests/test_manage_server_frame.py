...
from src.ui import ManageServerService, WorkOutcome
from src.ui.core_frames.manage_server_frame import ManageServerFrame


def _snapshot(*configs: ServerConfig) -> ServerConfigRegistrySnapshot:
    return ServerConfigRegistrySnapshot("test", tuple((config.name, config) for config in configs))
...
def test_build_server_refresh_payload_skips_empty_rows_and_preserves_order() -> None:
    server_data = [
        ["Alpha", "1.21", "Fabric", "執行中", "0 B", "已備份", "servers\\Alpha"],
        [],
    ...
    assert payload.server_rows["Alpha"] == tuple(server_data[0])
    assert payload.server_rows["Beta"] == tuple(server_data[2])


def test_build_server_refresh_payload_last_duplicate_name_wins_values() -> None:
    server_data = [
        ["Alpha", "1.21", "Fabric", "執行中", "0 B", "已備份", "servers\\Alpha"],
    ...
    assert payload.server_order == ["Alpha", "Alpha"]
    assert payload.server_rows["Alpha"] == tuple(server_data[1])


def test_build_server_refresh_payload_combines_order_and_rows() -> None:
    server_data = [
        ["Alpha", "1.21", "Fabric", "執行中", "0 B", "已備份", "servers\\Alpha"],
        ["Beta", "1.20.6", "Forge", "已停止", "0 B", "未備份", "servers\\Beta"],
    ]
...
def test_monitor_server_arms_ready_notification_for_new_start_without_forcing_focus() -> None:
    frame = _DummyFrame()
    frame.selected_server = "Alpha"
    fake_window = FakeMonitorWindow()
    frame._monitor_windows = {"Alpha": fake_window}

    ManageServerFrame.monitor_server(
        cast(Any, frame),
        bring_to_front=False,
        notify_when_ready=True,
    )

    assert fake_window.arm_calls == 1
    assert fake_window.show_calls == 1
    assert fake_window.raise_calls == 0
    assert fake_window.activate_calls == 0
...
def test_build_server_display_row_formats_vanilla_loader() -> None:
    ...
    assert row == ["Beta", "1.21.1", "1.21.1", "執行中", "0 B", "已備份", "servers\\Beta"]
...
def test_get_server_size_tolerates_per_file_stat_error(monkeypatch: Any, tmp_path: Path) -> None:
    server_path = tmp_path / "Alpha"
    server_path.mkdir()
    file1 = server_path / "good.txt"
    file2 = server_path / "bad.txt"
    file1.write_bytes(b"12345")
    file2.write_bytes(b"67890")

    original_stat = Path.stat

    def mocked_stat(self: Path, *args: Any, **kwargs: Any) -> Any:
        if self.name == "bad.txt":
            raise OSError("模擬檔案鎖定或無權限")
        return original_stat(self, *args, **kwargs)

    monkeypatch.setattr(Path, "stat", mocked_stat)
    assert ManageServerService._get_server_size(str(server_path)) == "5 B"


def test_server_size_cache_reuses_scan_until_cleared(tmp_path) -> None:
    service = _manage_service(SimpleNamespace(snapshot=lambda: ()))
    calls: list[str] = []
    ...
    service._get_server_size = scan

    assert service._get_cached_server_size(str(tmp_path)) == "8 B"
    assert service._get_cached_server_size(str(tmp_path)) == "8 B"
...
def test_begin_refresh_returns_monotonic_generation() -> None:
    fake_crud = SimpleNamespace(snapshot=lambda: ())
...
def test_accept_projection_returns_no_changes_when_unchanged() -> None:
    fake_crud = SimpleNamespace(snapshot=lambda: _snapshot())
    service = _manage_service(fake_crud)
...
def test_get_backup_status_uses_injected_backup_manager() -> None:
    ...
    fake_crud = SimpleNamespace(snapshot=lambda: _snapshot(config))
...
def test_backup_requires_secret_content_confirmation(monkeypatch) -> None:
    ...
    class _BackupFrame:
        ...
        server_backup = SimpleNamespace(backup_server=lambda *_args, **_kwargs: backend_calls.append("backup"))

        @staticmethod
        def window():
            return None

    def reject_backup(title, message, *_args, **_kwargs):
    ...
    assert prompts and prompts[0][0] == "確認備份"
    assert "RCON 密碼" in prompts[0][1]
...
def test_manage_server_frame_update_selection_disables_restore_when_running() -> None:
    ...
    frame.server_crud = SimpleNamespace(snapshot=lambda: _snapshot(config))
