...
import src.core.mods.dependency_plan_serializer as dependency_serializer_module
...
class _PlanningProvider:
    def find_projects(
        ...
        *,
        ...
        if isinstance(query, str):
            if search:
                return mod_search_provider_module._search_mods(
                    query,
            ...
            if include_details:
                return mod_search_provider_module._get_project_info(query)
            if exact:
                name = mod_search_provider_module._fetch_project_name(query)
                if name is not None:
                    return models_module.ProviderCatalogOutcome(
                        "found",
                        project_id=query,
                        display_name=name,
                        confidence=100,
                    )
                return models_module.ProviderCatalogOutcome("not_found")
            return models_module.ProviderCatalogOutcome("not_found")
        return mod_search_provider_module._resolve_project_names(query)
    ...
    def resolve_versions(
        ...
        *,
        ...
        if version_id is not None:
            return mod_search_provider_module._get_version_details(version_id)
        if recommended:
            return mod_search_provider_module._get_recommended_version(project_id, minecraft_version, loader)
        return mod_search_provider_module._get_versions(project_id, minecraft_version, loader)
    ...
    def resolve_files(
        ...
        *,
        ...
        if latest:
            return mod_search_provider_module._resolve_latest_versions_by_hashes(
                hashes, algorithm, minecraft_version, loader
            )
        return mod_search_provider_module._resolve_current_versions_by_hashes(hashes, algorithm)
...
def test__search_mods_maps_modrinth_hits(monkeypatch) -> None:
...
def test__search_mods_passes_category_facets(monkeypatch) -> None:
...
def test__search_mods_supports_browse_mode_without_query(monkeypatch) -> None:
    captured_params: dict[str, object] = {}
...
def test__search_mods_filters_out_pure_client_hits(monkeypatch) -> None:
    def fake_get_json(**_kwargs):
        return {
            "hits": [
                {
...
def test__get_versions_filters_and_selects_primary_file(monkeypatch) -> None:
    ...
    assert captured_params == {"game_versions": '["1.21"]', "loaders": '["fabric"]'}


def test__get_versions_requires_exact_quilt_loader(monkeypatch) -> None:
    def fake_get_json(**_kwargs):
        return [
            {
                "id": "ver-fabric",
...
def test__get_versions_requires_exact_neoforge_loader(monkeypatch) -> None:
    def fake_get_json(**_kwargs):
        return [
            {
                "id": "ver-forge",
...
def test__get_versions_skips_prerelease_entries(monkeypatch) -> None:
    def fake_get_json(**_kwargs):
        return [
            {
                "id": "beta1",
...
def test__get_recommended_version_returns_none_when_only_prerelease_exists(monkeypatch) -> None:
    def fake__get_versions(_project_id: str, _minecraft_version=None, _loader=None):
        return []

    _set_planning_dependency(monkeypatch, "_get_versions", fake__get_versions)

    assert (
        mod_search_provider_module._get_recommended_version("proj123", minecraft_version="1.21", loader="fabric")
        is None
    )


def test__get_versions_preserves_project_id_case_for_api(monkeypatch) -> None:
    captured_url = {"value": ""}
...
def test__resolve_latest_versions_by_hashes_posts_prism_style_payload(monkeypatch) -> None:
    ...
    results = mod_search_provider_module._resolve_latest_versions_by_hashes(
        ["abc123"],
...
def test__resolve_latest_versions_by_hashes_uses_exact_quilt_loader(monkeypatch) -> None:
    captured_request: dict[str, object] = {}
    ...
    mod_search_provider_module._resolve_latest_versions_by_hashes(
        ["abc123"],
        minecraft_version="1.21.1",
        loader="quilt",
    )
...
def test__resolve_latest_versions_by_hashes_uses_exact_neoforge_loader(monkeypatch) -> None:
    captured_request: dict[str, object] = {}
...
def test__search_mods_uses_exact_quilt_loader_facet(monkeypatch) -> None:
    captured_params: dict[str, object] = {}
...
def test_mod_planning_build_local_update_plan_marks_invalidated_stale_provider_as_blocked(monkeypatch) -> None:
    ...
    _set_planning_dependency(monkeypatch, "_resolve_current_versions_by_hashes", lambda *_args, **_kwargs: {})
    _set_planning_dependency(monkeypatch, "_resolve_latest_versions_by_hashes", lambda *_args, **_kwargs: {})
    _set_planning_dependency(monkeypatch, "_get_project_info", lambda *_args, **_kwargs: None)
    _set_planning_dependency(monkeypatch, "_search_mods", lambda *_args, **_kwargs: [])
...
def test_mod_planning_build_dependency_plan_resolves_version_id_dependency(monkeypatch) -> None:
    ...
    def fake__fetch_project_name(project_id: str) -> str | None:
        return "Cloth Config" if project_id == "cloth-config" else None

    def fake__get_versions(_project_id: str, _minecraft_version=None, _loader=None):
        return []

    _set_planning_dependency(monkeypatch, "_fetch_project_name", fake__fetch_project_name)
    _set_planning_dependency(monkeypatch, "_get_versions", fake__get_versions)
...
def test_mod_planning_build_local_update_plan_uses_resolved_online_project_id(monkeypatch) -> None:
    ...
    def fake__get_recommended_version(project_id: str, _minecraft_version=None, _loader=None):
        captured_project_ids.append(project_id)
        return

    _set_planning_dependency(monkeypatch, "_get_recommended_version", fake__get_recommended_version)
...
def test__search_mods_normalizes_filename_like_query(monkeypatch) -> None:
    captured_params: dict[str, object] = {}
...
def test_mod_planning_build_dependency_plan_collects_recursive_dependencies(monkeypatch) -> None:
    ...
    def fake__get_versions(project_id: str, minecraft_version=None, loader=None):
    ...
    _set_planning_dependency(monkeypatch, "_get_versions", fake__get_versions)
    _set_planning_dependency(monkeypatch, "_fetch_project_name", lambda project_id: str(project_id).title())
    _set_planning_dependency(
        monkeypatch,
        "_resolve_project_names",
...
def test_mod_planning_build_dependency_plan_preserves_dependency_project_id_case_for_api(monkeypatch) -> None:
    ...
    def fake__get_versions(project_id: str, _minecraft_version=None, _loader=None):
        captured_project_ids.append(project_id)
        return []

    def fake__resolve_project_names(_project_ids):
        return {"p7dr8msh": "Fabric API"}

    _set_planning_dependency(monkeypatch, "_get_versions", fake__get_versions)
    _set_planning_dependency(monkeypatch, "_resolve_project_names", fake__resolve_project_names)
...
def test_mod_planning_build_local_update_plan_prefers_hash_first_update_detection(tmp_path: Path, monkeypatch) -> None:
    ...
    _set_planning_dependency(
        monkeypatch,
        "_resolve_current_versions_by_hashes",
    ...
    def fake__resolve_latest_versions_by_hashes(
        _hashes,
        _algorithm="sha512",
    ...
    _set_planning_dependency(monkeypatch, "_resolve_latest_versions_by_hashes", fake__resolve_latest_versions_by_hashes)
    _set_planning_dependency(
        monkeypatch,
        "_get_recommended_version",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("fallback path should not be used")),
    )
    _set_planning_dependency(monkeypatch, "_resolve_project_names", lambda _project_ids: {"proj123": "Example Mod"})
    _set_planning_dependency(
        monkeypatch,
        "analyze_version",
        lambda *_args, **_kwargs: models_module.OnlineModCompatibilityReport(),
...
def test_mod_planning_build_local_update_plan_prefers_cached_local_hash(monkeypatch) -> None:
    ...
    _set_planning_dependency(
        monkeypatch, "_resolve_current_versions_by_hashes", lambda _hashes, _algorithm="sha512": {}
    )
...
def test_mod_planning_build_local_update_plan_allows_project_fallback_when_hash_mapping_missing(monkeypatch) -> None:
    ...
    _set_planning_dependency(monkeypatch, "_get_recommended_version", lambda *_args, **_kwargs: latest_version)
...
def test_mod_planning_build_local_update_plan_collects_metadata_summary(monkeypatch) -> None:
    ...
    _set_planning_dependency(monkeypatch, "_get_recommended_version", lambda *_args, **_kwargs: None)
...
def test_mod_planning_build_local_update_plan_defers_stale_revalidation_when_backoff_not_due(monkeypatch) -> None:
    ...
    _set_planning_dependency(monkeypatch, "_get_project_info", _count_project_info)
    _set_planning_dependency(monkeypatch, "_search_mods", _count_search)
...
def test_install_remote_mod_file_downloads_into_mods_dir(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_download_file(
        ...
        **_kwargs,
        ...
        assert expected_hash_algorithm == "sha256"
        ...
        return utils_module.OperationResult(True)
...
def test_install_remote_mod_file_rejects_existing_different_target(tmp_path: Path, monkeypatch) -> None:
    manager = ModManager(str(tmp_path))
    target_path = tmp_path / "mods" / "other-mod.jar"
    target_path.write_bytes(b"existing-mod")

    def fake_download_file(*_args, **_kwargs):
        raise AssertionError("a colliding target must be rejected before download")

    monkeypatch.setattr(utils_module.HTTPClient, "download_file", fake_download_file)
    replacement_hash = hashlib.sha256(b"replacement-mod").hexdigest()

    result = manager.mod_file_installer.install_remote_mod_file_result(
        download_url="https://example.invalid/replacement.jar",
        filename="other-mod.jar",
        expected_hash=replacement_hash,
    )

    assert result.status == "failed"
    assert result.message == "target_conflict"
    assert target_path.read_bytes() == b"existing-mod"


def test_install_remote_mod_file_rechecks_target_before_replace(tmp_path: Path, monkeypatch) -> None:
    manager = ModManager(str(tmp_path))
    target_path = tmp_path / "mods" / "race-mod.jar"
    replacement = b"replacement-mod"
    replacement_hash = hashlib.sha256(replacement).hexdigest()

    def fake_download_file(_url, local_path, **_kwargs):
        target_path.write_bytes(b"created-during-download")
        Path(local_path).write_bytes(replacement)
        return utils_module.OperationResult(True)

    monkeypatch.setattr(utils_module.HTTPClient, "download_file", fake_download_file)

    result = manager.mod_file_installer.install_remote_mod_file_result(
        download_url="https://example.invalid/race.jar",
        filename="race-mod.jar",
        expected_hash=replacement_hash,
    )

    assert result.status == "failed"
    assert result.message == "target_conflict"
    assert target_path.read_bytes() == b"created-during-download"


def test_replace_local_mod_file_allows_same_path_update(tmp_path: Path, monkeypatch) -> None:
    manager = ModManager(str(tmp_path))
    target_path = tmp_path / "mods" / "example.jar"
    ...
    def fake_download_file(_url, local_path, expected_hash=None, **_kwargs):
        assert expected_hash == replacement_hash
        Path(local_path).write_bytes(replacement)
        return utils_module.OperationResult(True)
...
def test_replace_local_mod_file_rejects_disabled_target_collision(tmp_path: Path, monkeypatch) -> None:
    manager = ModManager(str(tmp_path))
    old_path = tmp_path / "mods" / "old-mod.jar.disabled"
    colliding_path = tmp_path / "mods" / "new-mod.jar.disabled"
    old_path.write_bytes(b"old-mod")
    colliding_path.write_bytes(b"other-mod")
    local_mod = mod_manager_module.LocalModInfo(
        id="old-mod",
        name="Old Mod",
        ...
        minecraft_version="1.21",
        loader_type="Fabric",
        status=models_module.ModStatus.DISABLED,
        file_path=str(old_path),
    )

    def fake_download_file(*_args, **_kwargs):
        raise AssertionError("a colliding disabled target must be rejected before download")

    monkeypatch.setattr(utils_module.HTTPClient, "download_file", fake_download_file)

    replaced_path = manager.mod_file_installer.replace_local_mod_file(
        local_mod,
        "https://example.invalid/new-mod.jar",
        "new-mod.jar",
        expected_hash="a" * 64,
    )

    assert replaced_path is None
    assert old_path.read_bytes() == b"old-mod"
    assert colliding_path.read_bytes() == b"other-mod"
...
def test_install_remote_mod_file_rejects_missing_or_sha1_hash(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_download_file(*args, **kwargs):
        ...
        return utils_module.OperationResult(False, "unexpected download")
...
def test_install_remote_mod_file_cancellation_leaves_no_target_file(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_download_file(url, local_path, _progress_callback=None, expected_hash=None, cancel_check=None, **_kwargs):
        ...
        return utils_module.OperationResult(False, "下載已取消")
...
def test__get_recommended_version_does_not_fallback_for_unsupported_loader(monkeypatch) -> None:
    _set_planning_dependency(
        monkeypatch,
        "_get_versions",
    ...
    resolved = mod_search_provider_module._get_recommended_version(
        "example-project",
        minecraft_version="1.21.1",
        loader="paper",
    )
...
def test_mod_planning_build_local_update_plan_skips_online_update_check_for_unsupported_loader(monkeypatch) -> None:
    ...
    _set_planning_dependency(
        monkeypatch,
        "_resolve_latest_versions_by_hashes",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_mod_planning_build_local_update_plan_mixed_fault_hash_hit_plus_unresolved(monkeypatch) -> None:
    ...
    _set_planning_dependency(monkeypatch, "provider_identity_fixture", lambda _local_mod: None)
    _set_planning_dependency(monkeypatch, "_resolve_project_names", lambda _project_ids: {})
    _set_planning_dependency(
        monkeypatch,
        "analyze_version",
        lambda *_args, **_kwargs: models_module.OnlineModCompatibilityReport(),
...
def test_dependency_plan_persistence_payload_roundtrip_includes_provider_fields() -> None:
    ...
    payload = dependency_serializer_module.serialize_online_dependency_install_plan(
        plan,
    ...
    restored = dependency_serializer_module.deserialize_online_dependency_install_plan(payload)
    ...
    assert dependency_serializer_module.validate_online_dependency_install_plan_payload(payload) == (True, "ok")
