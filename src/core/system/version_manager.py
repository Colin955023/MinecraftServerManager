...
class MinecraftVersionManager(Singleton):
            ...
            if cache_path.exists():
                try:
                    ...
                    if age < 12 * 60 * 60:
                        ...
                        if cached_list:
                            valid_versions = [v for v in cached_list if self._has_valid_server_url(v)]
                            if valid_versions:
                                logger.info(
                                    f"Minecraft版本快取仍在有效期內（age={age:.1f}s），直接回傳 {len(valid_versions)} 個可用版本"
                                )
                                return valid_versions
                except Exception as e:
                    logger.debug(f"檢查版本快取有效期失敗: {e}")
                    ...
                    def fetch_server_url(v_obj):
                        try:
                            ...
                            if v_data:
                                ...
                                v_obj["server_sha1"] = server_info.get("sha1", "")
                            else:
                                v_obj["server_url"] = ""
                                v_obj["server_sha1"] = ""
                        except Exception as e:
    ...
    def get_server_download_info(self, version_id: str) -> tuple[str, str] | None:
        """獲取指定版本的伺服器下載 URL 與 SHA-1 雜湊。

        Args:
            version_id: Minecraft 版本 ID。

        Returns:
            (url, sha1) 元組；找不到時回傳 None。
        """
        try:
            versions = self.get_versions(force_fetch=False)
            target_ver = next((v for v in versions if v["id"] == version_id), None)
            if target_ver and target_ver.get("server_url"):
                return (target_ver["server_url"], target_ver.get("server_sha1", ""))
            return None
        except Exception as e:
            record_and_mark(
                e,
                marker_path=Path(self.cache_file),
                reason="get_server_download_info failed",
                details={"version_id": version_id},
            )
            logger.exception(f"獲取伺服器下載資訊失敗 {version_id}: {e}")
            return None
