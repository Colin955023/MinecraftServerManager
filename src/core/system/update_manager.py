...
logger = get_logger().bind(component="UpdateManager")
...
class UpdateManager:
    ...
    @staticmethod
    def _launch_installer(installer_path: Path, parent=None, interaction: Any | None = None) -> bool:
        """
        啟動安裝程式
        ...
        """
        installer_interaction: Any = interaction or ()
        try:
            try:
            ...
            if resolved_path.is_file():
                ...
                if not confirm:
                    ...
                    return False
                process = SubprocessUtils.popen_detached(UpdateManager._build_installer_launch_args(resolved_path))
                time.sleep(0.5)
    ...
    @staticmethod
    def _graceful_exit(parent, delay_ms: int = 100, interaction: Any | None = None) -> None:
        """
        優雅關閉應用程式
        ...
        Args:
            ...
            interaction: 更新流程互動介面。
        """
        exit_interaction: Any = interaction or ()
        try:
    ...
    def _clean_release_notes(body: str) -> str:
        ...
        text = "\n".join(kept_lines)
        decoded = UpdateManager._markdown_to_safe_text(text)
    ...
    def check_and_prompt_update(
        ...
        """
        update_interaction: Any = interaction or ()
        ...
        def _work() -> None:
            ...
            def _handle_checksum_mismatch(asset_name: str, algorithm: str) -> None:
                ...
                update_interaction.call_on_ui(
                    parent,
                    lambda: update_interaction.show_error(
                        "檔案雜湊驗證失敗",
            ...
            try:
                ...
                include_prerelease = UpdateManager._is_development_environment()
                latest = UpdateManager._get_latest_release(owner, repo, include_prerelease=include_prerelease)
                if not latest:
                    logger.info("無法從 GitHub 取得最新版本資訊")
                    if show_up_to_date_message:
                        update_interaction.call_on_ui(
                            parent,
                            lambda: update_interaction.show_info(
                                "檢查更新",
                ...
                latest_ver = UpdateManager._parse_version(latest_tag)
                current_ver = UpdateManager._parse_version(current_version)
                ...
                rendered = UpdateManager._clean_release_notes(body)
                html_url = latest.get("html_url")
                msg = f"發現新版本：{name}\n目前版本：{current_version}\n\n釋出說明：\n{rendered}\n\n是否下載並安裝？"
                result = update_interaction.call_on_ui(
                    parent,
                    lambda: update_interaction.ask_yes_no_cancel("更新可用", msg, parent=parent, show_cancel=False),
                )
                if not result:
                    return
                logger.info("使用者確認更新，準備下載...")
                asset, _ = UpdateManager._select_update_asset(latest)
                if not asset:
                    update_interaction.call_on_ui(
                        parent,
                        lambda: update_interaction.show_info(
                    ...
                    if html_url:
                        update_interaction.open_external(html_url)
                    return
                ...
                if HTTPUtils.download_file(
                    ...
                    installer_started = UpdateManager._launch_installer(
                    ...
                    _cleanup_temp_files(temp_files_to_cleanup)
                    return
                UpdateManager._graceful_exit(parent, interaction=update_interaction)
            except Exception as e:
                ...
                update_interaction.call_on_ui(
                    parent,
                    lambda: update_interaction.show_error(
                        "更新檢查失敗", f"無法完成更新檢查或下載：{error_msg}", parent=parent
                    ),
                )
                _cleanup_temp_files(temp_files_to_cleanup)

        update_interaction.run_async(_work)
