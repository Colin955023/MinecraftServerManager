...
MOD_INDEX_MAX_BYTES = 16 * 1024 * 1024


class ModIndexPersistence:
    """
    管理模組 JAR 檔案的增量索引
    ...
    def __init__(self, server_path: str, index_dir: str | None = None):
        ...
        if is_reparse_point(self.server_path):
            raise ValueError("模組索引伺服器路徑不可為符號連結或 reparse point")
        self.mods_path = self.server_path / "mods"
        if is_reparse_point(self.mods_path):
            raise ValueError("模組索引 mods 目錄不可為符號連結或 reparse point")
    ...
    def _load_index(self) -> None:
        ...
        with self._index_lock:
            if self.index_file.exists():
                try:
                    raw_payload = read_json(self.index_file, None, max_bytes=MOD_INDEX_MAX_BYTES)
                    if raw_payload is None:
                        raise ValueError(f"索引檔案超過安全大小上限 {MOD_INDEX_MAX_BYTES} bytes 或格式無效")
        ...
        with self._index_lock:
            try:
                ...
                if isinstance(cached, dict) and (
                    stat.st_size != cached.get("size") or stat.st_mtime != cached.get("mtime")
                ):
                    for key in (
                        "hashes",
                        "metadata",
