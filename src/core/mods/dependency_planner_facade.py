...
class DependencyPlanningService:
    ...
    def build_hooks(self) -> DependencyPlanHooks:
        ...
        Returns:
            依賴展開流程所需的 hook 組合
        """
        return DependencyPlanHooks(
            resolve_project_names=resolve_modrinth_project_names,
            resolve_dependency_entry=self.resolve_dependency_entry,
            select_dependency_best_version=self.select_dependency_best_version,
            analyze_dependency_best_version=self.analyze_dependency_best_version,
            extract_dependency_download_target=self.extract_dependency_download_target,
            make_dependency_install_item=self.make_dependency_install_item,
...
def build_required_dependency_install_plan(
    ...
    *,
    ...
    expand_required_dependency_install_plan(
        ...
        plan=plan,
        hooks=planning_service.build_hooks(),
...
def build_local_mod_update_plan(
    ...
    *,
    ...
    Args:
        ...
        provider_identity_resolver: provider identity owner 提供的解析 command
        hash_cache_writer: 核心索引提供的 hash 持久化 command
...
__all__ = ["build_local_mod_update_plan", "build_required_dependency_install_plan"]
