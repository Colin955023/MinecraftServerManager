...
class ServerPropertiesHelper:
    ...
    def get_property_descriptions(cls) -> types.MappingProxyType:
        ...
        cls._property_descriptions_cache = {
            ...
            "chat-spam-threshold-seconds": "玩家聊天訊息的防刷頻冷卻時間(秒)。",
            "command-spam-threshold-seconds": "玩家執行指令的防刷頻冷卻時間(秒)。",
...
class ServerPropertiesValidator:
    ...
    VALIDATION_RULES: ClassVar[dict[str, tuple[str, int | None, int | None, list[str] | None]]] = {
        ...
        "chat-spam-threshold-seconds": ("int", 0, None, None),
        "command-spam-threshold-seconds": ("int", 0, None, None),
