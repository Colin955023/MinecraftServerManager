...
from ...utils import PathUtils, SubprocessUtils, SystemUtils, get_logger, record_and_mark
from .. import ServerCommands, ServerDetectionUtils, ServerInstance

if TYPE_CHECKING:
    from .server_crud import ServerCRUD
    from .server_repository import ServerRepository

logger = get_logger().bind(component="ServerStartup")


class ServerStartup:
    """處理伺服器的啟動、停止與執行期狀態監控邏輯。"""

    STARTUP_CHECK_DELAY = 0.1
    OUTPUT_QUEUE_MAX_SIZE = 1000

    def __init__(self, repository: ServerRepository, crud: ServerCRUD):
    ...
    def _resolve_startup_script_for_run(self, config: ServerConfig, server_path: Path) -> Path | None:
        ...
        if script_path is not None:
            logger.debug(f"使用既有啟提腳本，啟動前確認 Java 路徑: {script_path}")
            ServerCommands.repair_startup_script_java_command(script_path, config)
            return script_path
        if not self.crud.create_launch_script(config):
    ...
    def get_server_info(self, server_name: str) -> dict | None:
        """
        獲取指定伺服器的詳細資訊，包括運行狀態、PID、CPU 使用率、記憶體使用量、運行時間等。

        Args:
            server_name: 伺服器名稱。

        Returns:
            dict | None: 伺服器的詳細資訊，如果找不到則返回 None。
        """
        try:
            if server_name not in self.repository.servers:
                return None
            ...
            try:
                properties = self.repository.load_server_properties(server_name)
                if properties:
    ...
    def send_command(self, server_name: str, command: str) -> bool:
        ...
        Returns:
            bool: 如果命令發送成功則返回 True，否則返回 False。
        """
        try:
            instance = self._get_running_instance(server_name)
            if instance is None:
            ...
            if self._write_process_command(process, command):
                ...
                return True
            logger.error(f"無法向伺服器 {server_name} 發送命令：stdin 不可用", "ServerStartup")
            return False
        except Exception as e:
            with contextlib.suppress(Exception):
    ...
    def get_server_log_file(self, server_name: str) -> Path | None:
        ...
        Returns:
            Path | None: 日誌檔案的路徑，如果找不到則返回 None。
        """
        try:
            if server_name not in self.repository.servers:
                return None
            server_config = self.repository.servers[server_name]
