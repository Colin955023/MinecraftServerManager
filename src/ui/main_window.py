...
class MinecraftServerManager:
    ...
    def set_servers_root(self, new_root: Optional[str] = None) -> None:
        ...
        if new_root:
            ...
            try:
                if stored and os.path.basename(os.path.normpath(stored)).lower() == "servers":
                    settings.set_servers_root(base_dir)
            except Exception:
                pass
        ...
        try:
            ...
            for widget in self.root.winfo_children():
                try:
                    ...
                    LogUtils.error(f"清理子視窗時發生錯誤: {e}\n{traceback.format_exc()}", "MainWindow")

        except Exception as e:
            LogUtils.error(f"清理資源時發生錯誤: {e}\n{traceback.format_exc()}", "MainWindow")
        finally:
            # 最後銷毀主視窗
            try:
                self.root.destroy()
            except Exception as e:
                LogUtils.error(f"銷毀主視窗時發生錯誤: {e}\n{traceback.format_exc()}", "MainWindow")
                # 強制退出
                sys.exit(0)
    ...
    def show_manage_server(self, auto_select=None) -> None:
        ...
        def _refresh_and_optionally_select() -> None:
            try:
                ...
                if auto_select and hasattr(self.manage_server_frame, "server_tree") and self.manage_server_frame.server_tree:
                ...
                LogUtils.error(f"切換到管理伺服器頁面後刷新失敗: {e}\n{traceback.format_exc()}", "MainWindow")
    ...
    def show_about(self) -> None:
        ...
        ctk.CTkLabel(scrollable_frame, text=f"版本 {APP_VERSION}", font=get_font(size=18), text_color=("#a0aec0", "#a0aec0")).pack(
            pady=(0, 20)
        )
...
特別感謝 PrismLauncher 與 MinecraftModChecker 開發團隊的開源貢獻！"""
    ...
    def complete_initialization(self, server_config: ServerConfig, init_dialog) -> None:
        ...
        try:
            ...
            LogUtils.error(f"初始化後讀取 server.properties 失敗: {e}\n{traceback.format_exc()}", "MainWindow")
...
class ServerInitializationDialog:
        ...
        try:
            if self.server_process and self.server_process.poll() is None:
                ...
                try:
                    ...
                    LogUtils.error_exc(f"等待程序終止逾時/失敗，改用 kill: {e}", "InitServerDialog", e)
    ...
    def _run_server(self) -> None:
        ...
        try:
            ...
            LogUtils.error(f"伺服器啟動失敗: {e}\n{traceback.format_exc()}", "ServerInitializationDialog")
    ...
    def _extract_java_command_from_bat(self, start_bat: Path) -> Optional[List[str]]:
        ...
        try:
            ...
            LogUtils.error_exc(f"提取 Java 命令失敗: {e}", "ServerInitializationDialog", e)
        return None
