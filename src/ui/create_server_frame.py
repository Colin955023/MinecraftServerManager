...
from ..core import LoaderManager, ServerCRUD
...
class CreateServerFrame(QtWidgets.QWidget):
    ...
    def get_system_memory_mb() -> int:
        ...
        try:
            ...
            with __import__("contextlib").suppress(Exception):
                ExceptionUtils.record_and_mark(e, marker_path=Path(__file__), reason="get_system_memory_failed")
            logger.bind(component="").error(
                f"無法獲取系統記憶體資訊: {e}\n{traceback.format_exc()}", "CreateServerFrame"
            )
            UIUtils.show_message("錯誤", f"無法獲取系統記憶體資訊: {e}", message_level="error")
            return 0
    ...
    def update_memory_warning(self) -> None:
        ...
        try:
            ...
            with __import__("contextlib").suppress(Exception):
                ExceptionUtils.record_and_mark(e, marker_path=Path(__file__), reason="update_memory_warning_failed")
    ...
    def apply_theme_styles(self) -> None:
        ...
        for label in self.findChildren(QtWidgets.QLabel):
            if label in special_labels:
                continue
            label.setStyleSheet(f"color: {resolve_color(Colors.TEXT_PRIMARY)};")
            ...
            try:
                ...
                with __import__("contextlib").suppress(Exception):
                    ExceptionUtils.record_and_mark(
                        e, marker_path=Path(__file__), reason="preload_loader_versions_failed"
                    )
                logger.bind(component="").error(
                    f"預載入載入器版本失敗: {e}\n{traceback.format_exc()}", "CreateServerFrame"
                )
    ...
    def create_buttons(self, parent) -> None:
        ...
        reset_button.setMinimumWidth(Sizes.BUTTON_WIDTH_SECONDARY)
    ...
    def reset_form(self):
        ...
        try:
            ...
            with __import__("contextlib").suppress(Exception):
                ExceptionUtils.record_and_mark(e, marker_path=Path(__file__), reason="reset_form_failed")
    ...
    def update_server_config_ui(self, _event=None) -> None:
        ...
        if loader_type == "Vanilla":
            ...
            self.loader_version_combo.addItem("無")
            self.loader_version_combo.setEnabled(False)
            self.loader_version_combo.setCurrentText("無")
            self.loader_version_var.set("無")
            return
        self.loader_version_combo.setEnabled(True)
        if not mc_version:
        ...
        try:
            ...
            def update_ui():
                try:
                    ...
                    with __import__("contextlib").suppress(Exception):
                        ExceptionUtils.record_and_mark(
                            e, marker_path=Path(__file__), reason="update_loader_versions_ui_failed"
                        )
                    logger.bind(component="").error(
                        f"更新載入器版本 UI 失敗: {e}\n{traceback.format_exc()}", "CreateServerFrame"
                    )
            ...
            with __import__("contextlib").suppress(Exception):
                ExceptionUtils.record_and_mark(e, marker_path=Path(__file__), reason="load_loader_versions_failed")
        ...
        try:
                ...
                if progress_dialog.cancelled:
                    self.ui_queue.put(
                        lambda: UIUtils.show_message(
                            "取消", "使用者已取消建立伺服器", parent_window, message_level="info"
                        )
                    )
                    return True
                return False
            ...
            try:
                ...
                def _show_download_error() -> None:
                    UIUtils.show_message(
                        "下載失敗", f"下載伺服器檔案失敗: {error_message}", self.window(), message_level="error"
                    )
    ...
    def download_server_files(self, config: ServerConfig, progress_dialog: ProgressDialog, server_path: Path) -> bool:
        ...
        if installer_url:
            checksum, _ = self.loader_manager._get_loader_installer_checksum(installer_url)
            if checksum is None:
                proceed = TaskUtils.call_on_ui(
                    self.window(),
        ...
        def do_download(cancel_token: CancellationToken | None = None):
            ...
            user_java_path = (getattr(self, "java_path_var", None) and self.java_path_var.get().strip()) or None
