...
class ProviderMetadataRevalidationService:
    ...
    def build_summary_notes(self) -> list[str]:
        ...
        if self.metrics.retryable_count > 0:
            notes.append(f"其中 {self.metrics.retryable_count} 個過期 metadata 重查失敗，已標記為可重試並暫停自動更新")
        ...
        if self.metrics.attempted_count > 0:
            ...
            notes.append(
                ...
                + (f"、自適應調整 {self.metrics.adaptive_adjustment_count} 次" if self.metrics.adaptive_enabled else "")
            )
