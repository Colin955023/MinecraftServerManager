...
def _fetch_modrinth_project_detail(project_id: str) -> dict[str, Any] | None:
    ...
    if not clean_project_id:
        return None
    response = fetch_modrinth_project_detail(clean_project_id, timeout=MODRINTH_PROJECT_DETAIL_TIMEOUT_SECONDS)
    if not isinstance(response, dict):
        logger.debug(f"Modrinth project detail 取得失敗: {clean_project_id}")
        return None
    return response
