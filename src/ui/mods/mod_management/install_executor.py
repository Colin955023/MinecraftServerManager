...
class ModManagementInstallExecutorMixin(ModManagementRuntimeBase):
    ...
    def _refresh_online_queue_button(self) -> None:
        """更新安裝清單按鈕文字"""
        try:
            logger.debug(f"刷新安裝清單按鈕: pending_count= {len(getattr(self, 'pending_online_installs', []))}")
        except Exception:
    ...
    def _add_pending_online_install(self, pending: PendingOnlineInstall) -> bool:
        ...
        logger.debug(f"準備加入安裝清單: key={pending_key} project={getattr(pending, 'project_name', '')}")
        remaining_items = [
            item
    ...
    def _install_online_version(
        ...
        if report is not None and (not getattr(report, "compatible", True)):
            UIUtils.show_message(
                "版本不相容", self._format_online_version_report(version, report), dialog, message_level="error"
            )
            return
    ...
    def _remove_selected_pending_online_installs(
        ...
        if review_root_keys:
            ...
            selected_root_keys = self._collect_selected_root_keys_from(queue_tree, None)
        ...
        if removed_count <= 0:
            UIUtils.show_message(
                "提示", "目前選取項目不可移除，請選擇模組根項目後再試一次", dialog, message_level="warning"
            )
            return
        logger.info(f"安裝清單移除選取項目: removed={removed_count} remaining={len(self.pending_online_installs)}")
        self._refresh_online_queue_button()
        dialog.destroy()
    ...
    def _install_pending_online_install_queue(
        ...
        def install_task(cancel_token: CancellationToken | None = None) -> None:
            ...
            try:
                ...
                self.ui_queue.put(
                    lambda msg=str(e): UIUtils.show_message(
