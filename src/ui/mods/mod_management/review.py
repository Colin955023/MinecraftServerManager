...
from ..mod_search_service.dependency_planner_facade import build_local_mod_update_plan
...
class ModManagementReviewMixin(ModManagementRuntimeBase):
    ...
    @staticmethod
    def _collect_selected_root_keys_from(tree: Any, valid_keys: set[str] | None) -> set[str]:
        ...
        for item in tree.selectedItems():
            ...
            while current_item:
                node_id = current_item.data(0, Qt.ItemDataRole.UserRole)
                if valid_keys is not None and node_id in valid_keys:
            ...
            if valid_keys is not None and (not matched_valid_key):
                continue
            if node_id:
                selected_root_keys.add(node_id)
        return selected_root_keys

    @staticmethod
    def _get_selected_review_key(tree: Any, valid_keys: set[str]) -> str:
    ...
    @staticmethod
    def _set_selected_advisory_dependency_items_enabled(tree: Any, entry_map: dict[str, Any], enabled: bool) -> bool:
        changed = False
        for item in tree.selectedItems():
            normalized_item_id = str(item.data(0, Qt.ItemDataRole.UserRole) or "").strip()
    ...
    @staticmethod
    def _configure_review_action_button(button: Any, review_entries: list[Any], action_label: str) -> None:
        runnable_enabled = ModManagementReviewMixin._count_enabled_runnable_entries(review_entries)
        button.setText(f"⬇️ {action_label} {runnable_enabled} 個已啟用項目")
        button.setEnabled(runnable_enabled > 0)
    ...
    def show_online_install_queue(self) -> None:
        ...
        try:
            queue_banner = BodyLabel(
                f"安裝清單：共 {len(review_entries)} 項（可安裝 {sum(1 for e in review_entries if e.actionable)}）"
            )
        ...
        def refresh_queue_tree() -> None:
            self._render_review_task_tree(
                queue_tree, self._build_online_review_task_nodes(review_entries), _column_count=6
            )
        ...
        def refresh_queue_status_banner() -> None:
            ...
            subtitle.setText(
                self._build_online_install_review_subtitle(
                    actionable_count,
                    blocked_count,
            ...
            overview_label.setText(
                self._format_review_overview_text(
                    review_entries,
                    review_nodes,
                    action_label="安裝",
        ...
        def refresh_queue_summary(_event=None) -> None:
            ...
            summary_box.setPlainText(self._format_pending_install_review_text(review_entry))
        ...
        queue_tree.itemSelectionChanged.connect(refresh_queue_summary)
        refresh_queue_tree()
        refresh_queue_summary()
        try:
            if review_entries and queue_tree.topLevelItemCount() == 0:
                logger.warning(f"安裝清單節點建構完成但 Treeview 未顯示任何項目：review_count={len(review_entries)}")
                with suppress(Exception):
                    queue_banner.setText(
                        f"清單顯示異常：應有 {len(review_entries)} 項，但列表未顯示請關閉後重新開啟安裝清單或查看右側摘要"
                    )
        except Exception:
            logger.exception("檢查 queue_tree child 時發生例外")

        try:
            if queue_tree.topLevelItemCount() == 0:
                summary_box.setReadOnly(False)
                summary_box.setPlainText("目前無法直接在列表中顯示安裝項目；以下為回退顯示：\n\n")
                for entry in review_entries:
                    ...
                    summary_box.append(f"- {name} — {version}\n")
                summary_box.setReadOnly(True)
        except Exception:
        ...
        install_button = self._create_review_action_button(
            ...
            _hover_color=Colors.BUTTON_SUCCESS_HOVER,
        ...
        def refresh_queue_action_button() -> None:
            actionable_count = sum(1 for entry in review_entries if entry.actionable)
            install_button.setText(f"⬇️ 安裝 {actionable_count} 個可安裝項目")
            install_button.setEnabled(actionable_count > 0)

        def refresh_queue_project_page_button(_event=None) -> None:
            selected_root_key = self._get_selected_review_key(queue_tree, review_root_keys)
            review_entry = review_entry_map.get(selected_root_key)
            project_page_button.setEnabled(
                bool(review_entry and self._resolve_pending_install_review_project_page_url(review_entry))
            )
        ...
        queue_tree.itemSelectionChanged.connect(refresh_queue_project_page_button)
        dialog.exec()
    ...
    def check_local_mod_updates(self) -> None:
        ...
        if not self.current_server or not manager:
            UIUtils.show_message("警告", "請先選擇伺服器後再檢查模組更新", self.parent, message_level="warning")
            return
        ...
        def check_task() -> None:
            try:
                ...
                self.ui_queue.put(
                    lambda msg=str(e): UIUtils.show_message("更新檢查失敗", msg, self.parent, message_level="error")
                )
    ...
    def _create_review_action_button(
        ...
        *,
        ...
        btn = PrimaryPushButton(text) if fg_color == Colors.BUTTON_SUCCESS or bold else PushButton(text)
    ...
    def _render_review_task_tree(self, tree: Any, nodes: list[ReviewTaskNode], _column_count: int) -> None:
        ...
        for node in nodes:
            parent_id = group_items.get(node.group_key) if node.parent_id is None else node_items.get(node.parent_id)
            if parent_id is None and node.group_key in group_items:
                parent_id = group_items[node.group_key]

            item = QTreeWidgetItem(parent_id if parent_id else tree)
            item.setText(0, node.title)
            for i, val in enumerate(node.values, start=1):
        ...
        if selected_key:
            selected_item = node_items.get(selected_key)
            if selected_item:
                selected_item.setSelected(True)
                tree.scrollToItem(selected_item)
        else:
            first_root = next((node.root_key for node in nodes if node.node_kind == "root"), "")
            if first_root and first_root in node_items:
                node_items[first_root].setSelected(True)
                tree.scrollToItem(node_items[first_root])
    ...
    def _create_review_shared_ui(self, _main_frame: Any, _wraplength: int) -> tuple[Any, Any]:
        """建立 Review 對話框中重複使用的概覽標籤與樹狀視圖容器"""
        return self._get_install_review_dialog_builder().create_review_shared_ui()
    ...
    def _show_local_update_review_dialog(self, update_plan: LocalModUpdatePlan, scope_text: str) -> None:
        ...
        if not review_entries:
            ...
            UIUtils.show_message("更新檢查", message, self.parent, message_level="info")
            return
        ...
        def refresh_update_tree() -> None:
            ...
            self._render_review_task_tree(update_tree, nodes, _column_count=5)
        ...
        def refresh_update_status_banner() -> None:
            ...
            local_group_counts = self._count_local_update_review_groups(review_entries)
            subtitle.setText(
                self._build_local_update_review_subtitle(
                    scope_text,
                    enabled_count,
                    local_group_counts["blocked"],
                    advisory_count=local_group_counts["advisory"],
        ...
        def refresh_update_summary(_event=None) -> None:
            ...
            if not review_entry:
                return
            summary_box.setReadOnly(False)
            summary_box.setPlainText(self._format_local_update_review_text(review_entry))
            summary_box.setReadOnly(True)
        ...
        update_tree.itemSelectionChanged.connect(refresh_update_summary)
        refresh_update_tree()
        ...
        def refresh_update_project_page_button(_event=None) -> None:
            ...
            review_entry = entry_map.get(selected_key)
            project_page_button.setEnabled(
                bool(review_entry and self._resolve_local_update_review_project_page_url(review_entry))
            )
        ...
        update_tree.itemSelectionChanged.connect(refresh_update_project_page_button)
