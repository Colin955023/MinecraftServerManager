...
class ModManagementQueueOps:
    ...
    def show_browse_context_menu(self, event) -> None:
        ...
        Args:
            event: 觸發選單的滑鼠事件
        """
        if self.controller.online_browse_presenter.browse_tree is None:
            return
        if hasattr(event, "x"):
            item = self.controller.online_browse_presenter.browse_tree.itemAt(event)
            if item is not None:
                self.controller.online_browse_presenter.browse_tree.setCurrentItem(item)
                item.setSelected(True)
        has_selection, _, _ = self._get_selected_online_mod_context()
        if not has_selection:
            return

        menu = RoundMenu(parent=self.controller.online_browse_presenter.browse_tree)
    ...
    def install_online_mod(self, _event=None) -> None:
        ...
        if not self.controller.mod_session.server or not manager:
            UIUtils.show_message("警告", "請先選擇伺服器後再安裝模組", self.controller.parent, message_level="warning")
            return
        has_selection, _, selected_mod = self._get_selected_online_mod_context()
        if not has_selection:
            UIUtils.show_message("警告", "請先從線上列表選取模組", self.controller.parent, message_level="warning")
            return
        if not selected_mod:
    ...
    def copy_online_mod_info(self) -> None:
        ...
        try:
            ...
            UIUtils.show_message(
                "複製失敗", f"無法將模組資訊複製到剪貼簿：{e}", self.controller.parent, message_level="error"
            )
            return
        self.controller.update_status("線上模組資訊已複製到剪貼簿")
    ...
    def _get_selected_online_mod_context(self) -> tuple[bool, str, Any | None]:
        ...
        if not self.controller.online_browse_presenter.browse_tree:
            return (False, "", None)
        selected_items = self.controller.online_browse_presenter.browse_tree.selectedItems()
        if not selected_items:
            return (False, "", None)
        item = selected_items[0]
        row = self.controller.online_browse_presenter.browse_tree.indexOfTopLevelItem(item)
        project_id = ""
        if item:
        ...
        mod = self.controller.mod_session.online_mod_by_project_id(project_id)
        if mod is None:
            mod = self.controller.mod_session.online_mod_at(row)
    ...
    def _refresh_online_results_summary(self) -> None:
        ...
        if self.controller.online_browse_presenter.browse_results_label:
            self.controller.online_browse_presenter.browse_results_label.setText(
                self._build_online_results_summary_text()
            )

    def _get_online_query_text(self) -> str:
        """取得目前線上模組輸入框文字"""
        presenter = self.controller.online_browse_presenter
        entry = presenter.browse_search_entry
        if entry is not None:
            return presenter.online_search_filter.normalize(entry.text())
        return presenter.online_search_filter.normalize(presenter.search_var.get() or "")
    ...
    def _build_online_browse_request(self) -> tuple[OnlineBrowseRequest | None, str | None]:
        ...
        sort_by = self.controller.online_browse_presenter.browse_sort_options.get(
            self.controller.online_browse_presenter.browse_sort_var.get(), "relevance"
        )
        return (
            OnlineBrowseRequest(
    ...
    def _is_browse_tab_active(self) -> bool:
        """判斷目前是否正在顯示線上瀏覽頁"""
        if not self.controller.notebook:
            return False
        try:
            return self.controller.notebook.currentIndex() == 1
        except Exception:
            return False
    ...
    def _show_version_install_dialog(
        ...
        """顯示版本選擇對話框"""
        dialog = ModalMSFluentWindow(self.controller.parent, show_buttons=False)
        ...
        open_button.clicked.connect(self.controller.review_ops.show_online_install_queue)
        button_layout.addWidget(open_button)

        project_page_url = resolve_project_page_url(
            urls=(getattr(mod, "homepage_url", ""), getattr(mod, "url", "")),
            identifiers=(getattr(mod, "slug", ""), getattr(mod, "project_id", "")),
        )
