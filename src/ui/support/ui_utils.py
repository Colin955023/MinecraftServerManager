...
def _find_explorer_path() -> str | None:
    ...
    if not windir:
        return None
    candidate = Path(windir) / "explorer.exe"
...
class UIUtils:
    ...
    def _play_message_sound(level: str) -> None:
        """根據訊息層級播放對應的 Windows 系統音效，不額外依賴 winsound extension"""
        sound_map = {
            "error": 0x00000010,  # MB_ICONHAND
            "warning": 0x00000030,  # MB_ICONEXCLAMATION
    ...
    def open_external_url(url: str) -> bool:
        """
        只透過瀏覽器開啟 HTTP(S) 網址，不交給本機檔案關聯程式
        ...
        try:
            ...
            logger.error(f"開啟網址失敗: {type(e).__name__}")
            return False
