...
from src.utils import CancellationToken, OperationCancelledError, get_logger, work_cancellation

from .qt_runtime import cancel_timer, invoke_later, is_qobject_alive
...
class _WorkRunnable(QtCore.QRunnable):
    ...
    def run(self) -> None:
        ...
        try:
            with work_cancellation(self._cancel_token):
                result = self._work()
            if self._cancel_token.is_cancelled():
                self._outcome_signal.emit(self._generation, WorkOutcome.cancelled())
            else:
...
class UIWorkScope(QtCore.QObject):
    ...
    _schedule_signal = QtCore.Signal(int, object, object)
    ...
    def __init__(self, parent: QtCore.QObject):
        ...
        self._timers: dict[object, QtCore.QTimer] = {}
        ...
        self._callbacks: dict[
            int,
            tuple[
                Callable[[WorkOutcome], None] | None,
                bool,
                str | None,
            ],
        ] = {}
        ...
        self._schedule_signal.connect(self._schedule_on_ui, QtCore.Qt.ConnectionType.QueuedConnection)
    ...
    def submit(
        ...
        *,
        ...
        Args:
            ...
            critical: 保留已開始的交易工作，關閉時等待其完成或自行回滾
    ...
    def _on_outcome_received(self, generation: int, outcome: WorkOutcome) -> None:
        ...
        if on_done and not self._draining:
            on_done(outcome)
    ...
    def schedule(self, delay_ms: int, callback: Callable[[], None], *, key: str | None = None) -> None:
        ...
        if not self._draining:
            if QtCore.QThread.currentThread() is self.thread():
                self._schedule_on_ui(delay_ms, callback, key)
            else:
                self._schedule_signal.emit(delay_ms, callback, key)

    @QtCore.Slot(int, object, object)
    def _schedule_on_ui(self, delay_ms: int, callback: Callable[[], None], key: object) -> None:
        if self._draining:
            return
        timer_key = key if key is not None else object()
        previous = self._timers.pop(timer_key, None)
        ...
        self._timers[timer_key] = invoke_later(delay_ms, completed, parent=self)
