    ...
    def enhance_local_mods(self) -> None:
        ...
        def enhance_single(mod):
            try:
                ...
                LogUtils.error(f"模組 {mod.filename} 資訊失敗: {e}\n{traceback.format_exc()}", "ModManagementFrame")
                ...
                def apply_ui_update() -> None:
                    try:
                        if ok:
                            ...
                            if hasattr(self, "enhanced_mods_cache") and isinstance(self.enhanced_mods_cache, dict):
                                if old_filename in self.enhanced_mods_cache and new_filename not in self.enhanced_mods_cache:
                                    self.enhanced_mods_cache[new_filename] = self.enhanced_mods_cache[old_filename]

                            # 更新該列的 status 與 mtime（其他欄位保持不動）
                            row_values = list(self.local_tree.item(item, "values") or [])
                            if row_values:
                                ...
                                try:
                                    ...
                                    pass
                                self.local_tree.item(item, values=tuple(row_values), tags=(mod_id,))
        ...
        try:
            ...
            if self.all_selected:
                ...
                try:
                    ...
                    LogUtils.error_exc(f"更新全選按鈕文字失敗: {e}", "ModManagementFrame", e)
            else:
                # 全選
                ...
                for idx, (base_name, tree_item_id) in enumerate(selected_pairs, start=1):
                    ...
                    if ok:
                        ...
                        def apply_row_update(
                            ...
                            try:
                                if not (hasattr(self, "local_tree") and self.local_tree and self.local_tree.winfo_exists()):
                                    return
                                row_values = list(self.local_tree.item(item_id, "values") or [])
                                if row_values:
                                    row_values[0] = "✅ 已啟用" if status == ModStatus.ENABLED else "❌ 已停用"
                                    try:
                                        mtime_val = os.path.getmtime(file_path) if file_path else None
                                        if mtime_val:
                                            row_values[6] = datetime.fromtimestamp(mtime_val).strftime("%Y-%m-%d %H:%M")
                                    except Exception:
                                        pass
                                    self.local_tree.item(item_id, values=tuple(row_values), tags=(mod_id,))
                            except Exception:
                                # 批量過程中 UI 更新失敗不阻塞主流程
                                pass
