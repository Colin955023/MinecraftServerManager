...
def _launch_installer(installer_path: Path) -> None:
    """啟動安裝程式（使用 subprocess 避免重複啟動）"""
    try:
        if installer_path.exists() and installer_path.is_file():
            popen_checked([str(installer_path)])
...
def check_and_prompt_update(
    ...
    def _work() -> None:
        try:
            ...
            # 嘗試將 Markdown 轉為 HTML，然後簡單轉回純文字以進行顯示
            rendered = body
            try:
                html_text = _markdown.markdown(body)
                # 移除 HTML 標籤，並 unescape HTML 實體
                text_only = re.sub(r"<[^<]+?>", "", html_text)
                rendered = _html.unescape(text_only)
            except Exception:
                rendered = body
                    ...
                    for a in assets:
                        try:
                            ...
                            if an.endswith((".sha256", ".sha256sum", ".sha512", ".sha512sum")):
                                with tempfile.NamedTemporaryFile(delete=False, prefix="msm_chk_", suffix=".txt") as tf:
                                    tfpath = tf.name
                ...
                try:
                    with path.open("rb") as f:
                        for chunk in iter(lambda: f.read(8192), b""):
                            h.update(chunk)
                    return h.hexdigest().lower() == hex_checksum.lower()
                except Exception:
                    return False

            # 如果是可攜式更新（asset 為 portable zip），採用 ZIP 解壓套用流程
            if is_portable_mode() and download_url and str(download_url).lower().endswith(".zip"):
                if not _call_on_ui(
                    lambda: UIUtils.ask_yes_no_cancel(
                        "可攜式更新可用",
                        ...
                        if not ok:
                            ...
                            _call_on_ui(
                                lambda: UIUtils.show_error(
                                    "驗證失敗",
                                    "可攜式更新檔的 checksum 驗證失敗，停止更新以避免損壞安裝。",
                                    parent=parent,
                                    topmost=True,
                                )
                            )
                ...
                try:
                    ...
                    _call_on_ui(
                        lambda: UIUtils.show_error("解壓失敗", "無法解壓下載的更新檔。", parent=parent, topmost=True)
                    )
                    return

                # 備份整個原始目錄與 .config/.log
                base = _get_portable_base_dir()
                cfg = base / ".config"
                lg = base / ".log"
                ...
                try:
                    ...
                    shutil.copytree(
                        ...
                        str(backup_dir),
                        dirs_exist_ok=True,
                        ignore=shutil.ignore_patterns(".config", ".log", ".portable"),
                    )

                    # 單獨備份 .config 與 .log
                    if cfg.exists():
                        shutil.copytree(cfg, backup_root / ".config", dirs_exist_ok=True)
                        logger.info("已備份 .config")
                    if lg.exists():
                        shutil.copytree(lg, backup_root / ".log", dirs_exist_ok=True)
                        logger.info("已備份 .log")
                except Exception as e:
                    error_msg = str(e)
                ...
                try:
                    cmd_exe = _shutil.which("cmd.exe") or "C:\\Windows\\System32\\cmd.exe"
                    subprocess.Popen([cmd_exe, "/c", "start", "", str(apply_bat)])
                except Exception:
            ...
            if HTTPUtils.download_file(download_url, str(dest)):
                ...
                try:
                    ...
                    if chk:
                        ...
                        if not ok:
                            ...
                            _call_on_ui(
                                lambda: UIUtils.show_error(
                                    "驗證失敗",
                                    "安裝程式的 checksum 驗證失敗，停止更新以避免損壞系統。",
                                    parent=parent,
                                    topmost=True,
                                )
                            )
                    ...
                    logger.exception("在安裝程式 checksum 驗證流程發生錯誤，將中止更新以避免風險")
                    _call_on_ui(
                        lambda: UIUtils.show_error(
                            "驗證錯誤",
