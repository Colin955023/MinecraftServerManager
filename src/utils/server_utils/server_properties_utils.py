...
class ServerPropertiesHelper:
    ...
    def get_property_descriptions(cls) -> types.MappingProxyType:
        ...
        cls._property_descriptions_cache = {
            ...
            "entity-broadcast-range-percentage": "實體廣播範圍百分比 (10-1000)控制實體距離玩家多近時才發送數據包越高可見越遠但增加延遲",
            ...
            "level-seed": "世界種子碼留空則隨機生成",
            "level-type": "世界生成類型 ID可省略 minecraft: 前綴 (minecraft:normal, minecraft:flat, minecraft:large_biomes, minecraft:amplified, minecraft:single_biome_surface)",
            ...
            "rate-limit": "玩家發送數據包的速率限制 (預設: 0) 0 為無限制超過限制的玩家會被踢出",
            ...
            "resource-pack-id": "資源包的 UUID用於客戶端識別資源包快取",
            ...
            "spawn-protection": "重生點保護半徑 (2x+1) (預設: 16) 非 OP 玩家無法破壞範圍內方塊0 為停用",
            ...
            "management-server-secret": "管理伺服器使用的密鑰留空時會由伺服器自動產生",
    ...
    @staticmethod
    def _normalize_properties_map(raw_props: Any) -> dict[str, str]:
        """將解析結果標準化為 `dict[str, str]`"""
        normalized: dict[str, str] = {}
        iterable: list[tuple[Any, Any]] = []
        if isinstance(raw_props, dict):
            iterable = [(key, value) for key, value in raw_props.items()]
        ...
        try:
            ...
            if prop_type == "enum":
                if allowed is not None and normalized_value not in allowed:
                    return (False, f"{prop_name}: 無效的值允許值為：{', '.join(allowed)}（目前：{value}）")
                return (True, "")
            if prop_type == "str":
                return (True, "")
        except ValueError:
