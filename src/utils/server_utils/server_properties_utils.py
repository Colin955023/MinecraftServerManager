...
from src.utils import PathUtils, atomic_write_text, get_logger
...
class ServerPropertiesHelper:
    ...
    def load_properties(file_path) -> dict[str, str]:
        """
        從 server.properties 檔案讀取屬性設定並解析為字典
    ...
    def save_properties(file_path, properties: dict[str, str]) -> bool:
        ...
        try:
            ...
            lines.append("")
            payload = "\n".join(lines)
            if not atomic_write_text(properties_file, payload, encoding="utf-8"):
                raise OSError("atomic_write_text server.properties failed")
            logger.debug(f"已儲存 server.properties: path={properties_file}, property_count={len(normalized_props)}")
            return True
        except Exception as e:
