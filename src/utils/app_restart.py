...
from . import UIUtils, get_logger, popen_checked

logger = get_logger().bind(component="AppRestart")


def _expand_windows_path(path: Path) -> Path:
    """
    ...
    try:
        ...
        while True:
            ...
            needed = GetLongPathNameW(p, buf, buf_len)
            if needed == 0:
                return path
            if needed > buf_len:
                buf_len = needed
...
def _get_executable_info() -> tuple[list[str], bool, Path | None]:
    ...
    try:
        ...
        if candidate.exists() and candidate.is_file():
            ...
            # 嘗試向上尋找 main.py
            found = _find_main_in_parents(Path(__file__).parent.parent, max_levels=6)
            if found:
                script_path = found
            else:
                # 最後嘗試從當前工作目錄向上尋找 main.py
                found2 = _find_main_in_parents(Path.cwd(), max_levels=6)
                if found2:
                    script_path = found2
    except Exception:
        script_path = None
    ...
    try:
        ...
        if is_frozen:
            ...
            if exe_candidate:
                ...
                if not exists:
                    try:
                        expanded = _expand_windows_path(exe_resolved)
        ...
        if script_path is not None:
            try:
                ...
                if not exists:
                    try:
                        expanded = _expand_windows_path(script_resolved)
                ...
                if not exists:
                    found = _find_main_in_parents(Path.cwd(), max_levels=6)
                    if not found and script_resolved is not None:
                        # 從 script_path 預期位置向上搜尋
                        found = _find_main_in_parents(script_resolved.parent, max_levels=6)
                    if found:
        ...
        if not supported:
            exe_fallback = _find_exe_fallback()
            if exe_fallback:
                ...
                if is_frozen:
                    ...
                    if script_ok:
                        ...
                        if not found and script_parent is not None:
                            found = _find_main_in_parents(script_parent, max_levels=6)
                    ...
                    process = popen_checked(use_cmd, cwd=target_cwd, creationflags=creation_flags)
...
def schedule_restart_and_exit(parent_window=None, delay: float = 1.0) -> None:
    ...
    try:
        ...
        if restart_initiated:
            ...
            try:
                _supported, details = get_restart_diagnostics()
            except Exception:
