...
class LogUtils:
    ...
    def _initialize_log_file() -> None:
        ...
        try:
            ...
            # 建立日誌檔案名稱（使用日期）
            log_filename = f"minecraft_server_manager_{datetime.now().strftime('%Y%m%d')}.log"
            LogUtils._log_file = log_dir / log_filename
            
            # 檢查日誌檔案大小，如果過大則輪轉
            if LogUtils._log_file.exists() and LogUtils._log_file.stat().st_size > LogUtils._max_log_size:
                # 重命名舊檔案
