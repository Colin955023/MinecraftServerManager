...
class ServerPropertiesHelper:
    ...
    def load_properties(file_path) -> Dict[str, str]:
        ...
        try:
            ...
            LogUtils.error_exc(f"載入 server.properties 失敗: {e}", "PropertiesHelper", e)
    ...
    def save_properties(file_path, properties: Dict[str, str]):
        ...
        try:
            ...
            LogUtils.error_exc(f"儲存 server.properties 失敗: {e}", "PropertiesHelper", e)
            ...
            try:
                ...
                if script_modified:
                    try:
                        ...
                        LogUtils.error_exc(f"無法重寫腳本 {fpath}: {e}", "ServerDetection", e)
            except Exception as e:
                LogUtils.error_exc(f"解析啟動腳本失敗 {fpath}: {e}", "ServerDetection", e)
        ...
        for args_file in ["user_jvm_args.txt", "jvm.args"]:
            ...
            if fpath.exists():
                try:
                    ...
                    LogUtils.error_exc(f"解析 JVM 參數檔失敗 {fpath}: {e}", "ServerDetection", e)
    ...
    def detect_loader_and_version_from_sources(server_path: Path, config, loader: str) -> None:
        ...
        def detect_from_version_json():
            ...
            try:
                ...
                LogUtils.error_exc(f"解析 version.json 失敗 {fp}: {e}", "ServerDetection", e)
