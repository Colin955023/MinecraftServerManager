...
class LoggerConfig:
    ...
    def initialize(cls) -> None:
        ...
        try:
            ...
            logger.bind(component="Logger").info(
                f"日誌系統初始化完成，日誌檔案：{log_file_path}"
            )
    ...
    def _get_folder_size_mb(cls, folder: Path) -> float:
        ...
        try:
            # 使用生成器表達式和 sum 優化效能
            total_size = sum(
                f.stat().st_size for f in folder.glob("*.log") if f.is_file()
            )
    ...
    def _cleanup_old_logs_if_needed(cls) -> None:
        ...
        try:
            ...
            if folder_size > cls._max_folder_size_mb:
                ...
                log_files = sorted(
                    cls._log_dir.glob("*.log"), key=lambda f: f.stat().st_mtime
                )
                ...
                for log_file in log_files:
                    ...
                    try:
                        ...
                        logger.bind(component="Logger").warning(
                            f"無法刪除舊日誌檔案 {log_file}: {e}"
                        )
