...
> 介面使用 PySide6 + QFluentWidgets (Fluent Design)；可見控制項採 QFluentWidgets，Qt 僅保留必要的結構／事件／model-view 基礎設施。顯示縮放跟隨 Windows 與 Qt 高 DPI 行為。
...
- **模組匯出** — 支援匯出模組清單為 Excel (.xlsx)、JSON、HTML 或純文字檔
- **匯入伺服器** — 掃描既有資料夾或壓縮檔快速匯入與批次探索
- **備份與還原** — ZIP 備份、歷程管理與安全覆寫還原（不刪除備份中不存在的既有檔案）
- **屬性視覺化** — 分頁視覺化編輯 `server.properties`，支援即時校驗與官方預設還原
