...
2. **伺服器名稱**：輸入一個易於識別的名稱（例如：Survival_1.20.4）。
   - 小提醒：若您已手動在名稱後面加上自訂文字，切換 MC 版本或載入器時，系統會保留這段自訂尾字。
...
6. **記憶體設定**：
   - **最小記憶體（選填）**：可留空讓 Java 自動決定。若留空，Java 會根據系統配置自動調整初始堆大小，只受最大記憶體限制。
   - **最大記憶體**：根據您的電腦配備設定。
...
- **強制停止**：若伺服器無回應，可強制終止程式。
- **欄位自動調寬**：在伺服器清單標題列上，雙擊欄位標題或欄位分隔線，可自動調整欄寬到目前內容最適大小。
