...
- 無需安裝，可複製到 USB 隨身碟使用
- **⚠️ 手動更新**：程式啟動時不會自動檢查更新。需要更新時請執行 `update-portable.bat`，腳本會自動檢查並安裝最新版本
...
> **更新提示**：
> - **安裝版本**：程式啟動時會自動檢查 GitHub 是否有新版本，若有則彈出通知視窗引導您下載更新。
> - **可攜式版本**：程式啟動時不會自動檢查更新。用戶需手動執行資料夾中的 `update-portable.bat` 以檢查並安裝新版本（腳本會自動從 GitHub 下載並覆蓋舊版本）。
