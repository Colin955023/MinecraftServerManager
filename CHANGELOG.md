...
- **伺服器備份與還原安全性**：備份改為暫存 ZIP 完整寫入後原子提交，並自動保留最新 10 份；還原採 staging + rollback 的快照替換，保留 `logs`、`crash-reports`、`backups`、`.git` 等排除目錄，提交失敗時回滾原目錄。
- **工具層精簡與分工**：解耦舊版 `path_utils.py`，拆分為專責之 `filesystem_utils.py`、`archive_utils.py` 與 `json_utils.py`。
- **單一執行檔自動更新**：下載單一執行檔並校驗 SHA256，確認後於背景替換目前版本並自動重啟。
