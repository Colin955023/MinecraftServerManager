...
- **伺服器大小資訊**：管理列表顯示整個伺服器目錄的總大小，重新整理時重新計算所有檔案，不限於 `world`。
- **模組相依與身分判定**：提供相容性／依賴規劃、Modrinth 身分解析與本地更新判定。

### 調整
- **使用者介面全面升級**：主視窗、伺服器管理、模組管理、設定與對話框統一採用 QFluentWidgets，支援一致的深淺色主題與操作回饋。
- **背景工作生命週期統一**：背景操作集中管理取消、完成回呼、錯誤處理與按鈕狀態，降低視窗關閉或切換時的競態。
- **伺服器管理職責分工**：建立、匯入、唯讀檢查、程序執行、設定檔、備份與登錄資料分離協作。
- **模組管理流程整理**：搜尋、相容性分析、依賴規劃、安裝審查與本地狀態採用一致的資料流程。
...
### 重大變更
- **核心架構與命名空間更迭**：舊有伺服器管理、啟動與路徑工具責任改由新版專責元件與 facade 取代。
- **UI 底層全面遷移**：移除 Tkinter 風格適配器與舊版相容層，全面採用 QFluentWidgets 與統一的背景工作排程。
