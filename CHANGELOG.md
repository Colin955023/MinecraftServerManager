...
### 調整
- **全專案架構重構至 C#／.NET 10／WPF**：主程式全面由 Python（PySide6）重構並遷移至 C#、.NET 10 與 WPF 原生技術棧，在完整維持所有伺服器生命週期管理、載入器一鍵安裝、模組管理、屬性編輯與交易式備份還原等核心功能等價（Parity）的前提下，全面提升系統效能、穩定性與安全可靠度
- **發布規格現代化（雙版本單一執行檔）**：全面捨棄 Python 打包工具，改採 .NET 官方原生 Single-File 單檔封裝與微軟官方二進位壓縮（無加殼、無暫存目錄釋放、零防毒軟體誤報），並提供「獨立自包含版（`MinecraftServerManager-self-contained.exe`，約 62 MB，開箱即用）」與「框架相依版（`MinecraftServerManager-framework-dependent.exe`，約 1.6 MB，極致精簡）」兩種規格
- **冷啟動速度與記憶體開銷大幅改善**：擺脫 Python 解譯器啟動與龐大動態連結庫載入負擔，冷啟動速度提升 4 至 8 倍，常駐記憶體佔用降低 70% 以上
- **Java 程序樹生命週期管理強化**：伺服器程序控制改採 Windows 核心原生 API 與 `entireProcessTree: true` 樹狀終止機制，徹底解決 Python 時代因行程中斷導致 `java.exe` 殘留為背景孤兒程序的長期問題
- **平行化模組處理與雜湊計算**：本地模組掃描改採 `Parallel.ForEachAsync` 充分發揮多核心效能，大量模組讀取與 SHA-512 雜湊運算速度較 Python 提升數倍
- **高頻日誌管線與主控台流暢度改善**：即時監控改採非同步 Task 抽水、50ms 微批次聚合緩衝器與 `[GeneratedRegex]` 編譯期正則，徹底杜絕伺服器高負載噴出大量日誌時使用者介面卡頓的問題
- **更新檢查安全防呆機制**：更新檢查服務新增本機 .NET 10 Desktop Runtime 探測能力，預設推薦相容性最高的自包含版，並在使用者電腦未安裝 .NET 10 時強制封鎖框架相依版的直接下載，杜絕下載後無法執行的狀況
...
- **伺服器設定舊版遷移機制**：匯入舊版伺服器時自動檢測舊版屬性與型別，提供相容性升級選項並自動備份原始 `server.properties`。
- **外部備份位置管理**：可為每個伺服器設定外部備份資料夾；後續備份直接沿用已儲存位置，刪除伺服器時可選擇是否一併刪除受管理備份。
- **模組清單匯出**：支援 Excel（XLSX）格式，並保留純文字、JSON 與 HTML 格式。
- **模組相依與身分判定**：提供相容性與依賴規劃、Modrinth 身分解析與本地更新判定。
- **伺服器大小資訊**：伺服器管理列表顯示整個伺服器目錄的總大小，涵蓋所有檔案而不限於世界資料檔。
- **無根目錄 JAR 壓縮檔匯入**：支援透過各類載入器啟動腳本自動解析並匯入沒有根目錄 JAR 的伺服器壓縮檔。
- **離線啟動環境支援**：快取 Minecraft 所需之 Java 主要版本，確保無網際網路連線時伺服器仍可正常啟動。

### 調整
- **使用者介面全面升級**：主視窗、伺服器管理、模組管理、設定與對話框統一採用 Fluent Design，支援一致的深淺色主題切換與操作回饋。
- **伺服器交易協調機制**：伺服器建立、匯入、備份、還原與刪除全面導入階段式交易管理與鎖定防護，支援操作失敗回滾與墓碑暫存目錄清理。
- **伺服器設定與衝突檢查**：保留未知與空白屬性，儲存前檢查 `server.properties` 是否被外部修改；匯入 Forge／NeoForge 伺服器時保留 `user_jvm_args.txt` 中的自訂啟動參數。
- **備份與還原流程強化**：採用完整 ZIP 寫入、原子提交與回滾保護機制，在備份、還原與刪除期間鎖定伺服器維護操作，避免並行操作衝突。
- **伺服器刪除安全防護**：刪除伺服器時防止誤刪執行中伺服器，改採墓碑暫存目錄隔離以確保資料安全。
- **原地匯入保護**：原地匯入伺服器時，若取消操作或匯入失敗，自動還原原地原始設定與檔案，避免誤清理資料。
- **更新與監控體驗改善**：更新套件下載時驗證 SHA256；大型主控台僅載入最新內容以提升流暢度；更新提示保留 Markdown 格式與程式碼排版。
- **啟動腳本保護**：清理過期伺服器啟動腳本時，自動保留管理員自訂之輔助維護腳本（如備份或維護腳本）。

### 修正
- 修正主視窗切換至其他視窗後再返回時，底部操作按鈕可能出現畫面殘影的問題。
- 修正監控視窗重新取得焦點時被強制取消最大化、出現白色外框，以及最大化按鈕暫時失效的問題。
- 修正伺服器啟動檔案突然消失時，啟動狀態可能卡在啟動中的問題。
- 修正伺服器主控台串流輸出發生例外時誤判程序已結束，進而誤開放進入維護模式的問題。
- 修正伺服器建立流程中 legacy Forge 安裝後的版本化啟動目標因路徑比對異常導致誤判失敗回滾的問題。
- 修正伺服器重新檢測後遺失已設定的外部備份路徑之問題。
- 修正伺服器記憶體設定未能正確持久化寫入至啟動腳本的問題。
...
- 修正模組依賴遞迴、版本 ID 大小寫、載入器與平台相容性顯示不一致的問題。
...
- 修正重複選擇備份目錄及外部備份誤被伺服器資料夾操作影響的問題。
- 修正載入器、模組識別與更新資料異常時的相容性判定與錯誤處理。
