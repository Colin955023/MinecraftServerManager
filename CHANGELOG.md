...
### 新增
- **伺服器建立確認流程**：建立前顯示已驗證的伺服器設定、警告與完整 JVM 啟動指令，確認後才執行建立。
- **JVM 與記憶體視覺化設定**：支援 JVM 參數與記憶體語法驗證，並依 Java 版本提供 ZGC／G1GC 建議。
- **模組清單匯出**：支援 Excel（XLSX）格式，並保留純文字、JSON 與 HTML 格式。
- **伺服器大小資訊**：管理列表顯示整個伺服器目錄的總大小，涵蓋所有檔案而不限於 `world`。
- **模組相依與身分判定**：提供相容性／依賴規劃、Modrinth 身分解析與本地更新判定。

### 調整
- **使用者介面全面升級**：主視窗、伺服器管理、模組管理、設定與對話框統一採用 QFluentWidgets，支援一致的深淺色主題與操作回饋。
- **伺服器設定與衝突檢查**：保留未知／空白屬性，儲存前檢查 `server.properties` 是否被外部修改。
- **備份與還原流程強化**：採用完整 ZIP 寫入、原子提交、保留策略與 staging／回滾流程，並在備份、還原、刪除期間鎖定伺服器維護操作。
- **更新與監控體驗改善**：更新套件驗證 SHA256；大型控制台僅載入最新內容；外部更新訊息以純文字顯示並保留程式碼內容。

### 修正
- 修正主視窗切換至其他視窗後再返回時，底部操作按鈕可能出現畫面殘影的問題。
- 修正監控視窗重新取得焦點時被強制取消最大化、出現白色外框，以及最大化按鈕暫時失效的問題。
- 修正伺服器啟動檔案突然消失時，啟動狀態可能卡在啟動中的問題。
- 修正備份、還原與刪除可能和伺服器執行或其他維護操作並行的問題。
- 修正受管理備份未依封存內容套用 ZIP 解壓縮大小限制，以及失敗時留下不完整檔案的問題。
- 修正更新提示移除 Markdown 程式碼內容、Java 下載錯誤訊息不完整與部分錯誤回報不明確的問題。
- 修正伺服器建立或匯入失敗時殘留暫存目錄、未完成註冊資料或路徑穿越安全問題。
- 修正模組依賴遞迴、版本 ID 大小寫、載入器／平台相容性與模組描述顯示不一致的問題。
- 修正模組審查選取狀態、可選依賴與過期結果可能被誤執行的問題。
- 修正伺服器子處理程序、PID、主控台串流輸出、停止狀態與玩家清單更新不同步的問題。
- 修正網路下載逾時、磁碟空間不足、無效連結與 Java 設定寫入失敗時的錯誤處理。
- 修正關閉主程式後背景程序偶爾仍殘留，現在會先停止伺服器與背景工作再離開。
